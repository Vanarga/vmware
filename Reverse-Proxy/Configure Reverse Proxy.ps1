<#
	...
	.PARAMETER Username
		AD Credential username with permission on vCenter and ability to connect to Certificate Authority.
	.PARAMETER Password
		Password for AD Credential.
	.PARAMETER IP
...
# Connect to vCenter.
$viHandle = Connect-VIServer $vCenter -User $Username -Password $Password

# Log in as root and change the password.
Set-VMKeystrokes -VMName $Deployment -StringInput "root" -ReturnCarriage $true
Set-VMKeystrokes -VMName $Deployment -StringInput 'changeme' -ReturnCarriage $true
Set-VMKeystrokes -VMName $Deployment -StringInput 'changeme' -ReturnCarriage $true
Set-VMKeystrokes -VMName $Deployment -StringInput $rootpassword -ReturnCarriage $true
Set-VMKeystrokes -VMName $Deployment -StringInput $rootpassword -ReturnCarriage $true
...
CopyFiletoServer $FileLocations $Deployment "root" $rootpassword $viHandle $true
...
$CommandList += "/usr/bin/script -c `"/root/config.sh " + $Username + " `'" + $Password + "`' " + $Deployment + " " + $IP + " " + $NetMask + " " + $Gateway + "`" /root/output.log"

# Excute the commands in $CommandList on the vcsa.
ExecuteScript $CommandList $Deployment "root" $rootpassword $viHandle

# Send via VMKeystrokes command to update packages. This reinstalls vmware tools so it cannot be done via ExecuteScript.
Set-VMKeystrokes -VMName $Deployment -StringInput "time tdnf -y distro-sync" -ReturnCarriage $true
