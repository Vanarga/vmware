...
Functions start at line 152
Main program starts at line 1090
...
26. TransferCertToNode			  Y		ExecuteScript, CopyFiletoServer
27. UserPEMFiles				  Y		CreatePEMFiles
28. VMCAMint					  N
29. CDDir						  Y
30. CreateVCSolutionCert		  Y		CreateSolutionCSR, OnlineMint, CreatePEMFiles
31. CreatePscSolutionCert		  Y		CreateSolutionCSR, OnlineMint, CreatePEMFiles
...
function ConfigureAutoDeployRules ($rules, $path, $vc) {
	...
	foreach ($rule in $rules) {
		...
		$hpMgr = Get-View -Server $vc -Id $si.Content.HostProfileManager
		...
		$pro = Get-VMHostProfile -Server $vc | ?{$_.Name -ieq $rule.ProfileName}
		$clu = Get-Cluster -Server $vc | ?{$_.Name -ieq $rule.Cluster}
...
function ConfigureLicensing ($Licenses, $vc) {
	echo $Licenses | Out-String
	Foreach ($License in $Licenses) {
		# Add License Key
		$LicMgr  = Get-View -Server $vc ServiceInstance
		$AddLic  = Get-View -Server $vc $LicMgr.Content.LicenseManager
		...
		If ($LicType.Name -ilike "*vcenter*") {
			...
			$licAssignMgr	= Get-View -Server $vc $AddLic.licenseAssignmentManager
			$licAssignMgr.UpdateAssignedLicense($vcUuid, $License.LicKey, $vcDisplayName)
		}
		Else {
			  # Assign Esxi License
			  $licDataMgr = Get-LicenseDataManager -Server $vc
			  for ($i=0;$i -lt $License.ApplyType.Split(",").count;$i++) {
				   switch ($License.ApplyType.Split(",")[$i]) {
					 CL {$viContainer = Get-Cluster -Server $vc -Name $License.ApplyTo.Split(",")[$i]}
					 DC {$viContainer = Get-Datacenter -Server $vc -Name $License.ApplyTo.Split(",")[$i]}
					 VH {$viContainer = Get-VMHost -Server $vc -Name $License.ApplyTo.Split(",")[$i]}
				   }   
...
function Deploy ($parameterlist, $ovftoolpath) {
	...
	if ($parameterlist.Action -ine "--version") {
		...
		if ($parameterlist.Action -ine "first" -and $pscs -contains $parameterlist.DeployType) {
