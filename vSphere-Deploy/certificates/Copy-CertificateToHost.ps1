function Copy-CertificateToHost {
	...
	Invoke-ExecuteScript $script $Deployment.Hostname "root" $Deployment.VCSARootPass $VIHandle
    ...
    $thumbPrint = $(Invoke-ExecuteScript $commandList $Deployment.Hostname "root" $Deployment.VCSARootPass $VIHandle).Scriptoutput.Split("=",2)[1]
